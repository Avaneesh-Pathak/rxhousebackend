...
const FRONTEND_DIR = path.resolve(__dirname, "../medicine");

console.log("=================================");
console.log("FRONTEND DIRECTORY:", FRONTEND_DIR);
console.log("BLOG TEMPLATE:");
console.log(
    path.join(FRONTEND_DIR, "blog-post.html")
);
console.log(
...
// Helper function to escape HTML to prevent injection in emails
...
app.post('/api/social-clicks', async (req, res) => {
    ...
    try {
        const result = await pool.query(
            'INSERT INTO social_clicks (platform, fullDate, date, page, device, browser) VALUES ($1,$2,$3,$4,$5,$6) RETURNING id', 
            [platform, fullDate || new Date().toISOString(), new Date().toLocaleString(), page || '', device || '', browser || '']
        );
        res.json({ id: result.rows[0].id });
    } catch (err) { 
        res.status(500).json({ error: 'Unable to save social click' }); 
    }
});

app.get('/api/social-clicks', async (req, res) => {
    try { 
        const { rows } = await pool.query('SELECT * FROM social_clicks ORDER BY id DESC LIMIT 200'); 
        res.json(rows); 
    } catch (err) { 
        res.status(500).json({ error: 'Unable to fetch social clicks' }); 
    }
});
...
app.post('/api/orders', async (req, res) => {
    ...
    try {
        await pool.query(
            'INSERT INTO orders (id,billing,itemCount,subtotal,shipping,tax,total,date) VALUES ($1,$2,$3,$4,$5,$6,$7,$8) ON CONFLICT (id) DO NOTHING', 
            [orderId, billing || {}, itemCount, subtotal, shipping, tax, total, date || new Date().toISOString()]
        );
        ...
        <p>
        Notes : ${escapeHtml(billingData.notes) || "None"}
        </p>
        `;
        ...
        if (billingData.email) {
            await transporter.sendMail({
                ...
                subject: "Order Confirmation - Pharmacies Doctor",
                html: `
        ...
        res.status(500).json({ error: 'Unable to save order', detail: err.message }); 
    }
});
...
app.get('/api/orders', async (req, res) => {
    try {
        ...
        const itemsByOrder = itemsRes.rows.reduce((acc, it) => { 
    ...
    try {
        ...
        await transporter.sendMail({
            ...
            html: `
                ...
                <p><b>Name:</b> ${escapeHtml(first_name)} ${escapeHtml(last_name)}</p>
                <p><b>Email:</b> ${escapeHtml(email)}</p>
                <p><b>Phone:</b> ${escapeHtml(phone) || 'Not provided'}</p>
                <p><b>Subject:</b> ${escapeHtml(subject)}</p>
                <p><b>Message:</b></p>
                <blockquote style="background:#f4f4f4; padding:10px;">${escapeHtml(message).replace(/\n/g, '<br>')}</blockquote>
            `
        });
...
app.get("/debug-blog", (req, res) => {
    console.log("DEBUG BLOG ROUTE HIT");
    console.log("DIRNAME:", __dirname);

    const templatePath = path.resolve(
        __dirname,
        "../medicine/blog-post.html"
    );
...
app.get("/blog/:slug", async (req, res) => {
    ...
    console.log("=================================");
    console.log("BLOG SSR ROUTE HIT");
    ...
    try {
        ...
        console.log("Database result:", rows.length);
        ...
        if (!fs.existsSync(templatePath)) {
            return res.status(500).send(
                "Blog post HTML template not found on server."
            );
        }
        ...
        if (blog.featured_image) {
            ...
            if (youtubeId) {
                ...
                image =
                    `${imageBase}/${blog.featured_image.replace(/^\/+/, "")}`;
            }
        }
